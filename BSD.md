...
:::info
### Sage
For elliptic curves over $\Q$ only:
...
## Field factor
This is $$|d_K|^{1/2}.$$  Gross includes this in with the local factors at infinite places.   For Abelian varieties of dimension $g$ the factor is $|d_K|^{g/2}$.
...
The expression given here will only be correct if the points $P_1,\dots,P_r$ are **saturated**, i.e. generate all of $E(K)$ modulo torsion, so that $[E(K):\left<P_1,\dots,P_r\right>] = |T|$. An alternative expression is sometimes seen which avoids this:  $$\frac{R(P_1,\dots,P_r)}{[E(K):\left<P_1,\dots,P_r\right>]^2};$$  this is a neat trick, but not very useful in practice for computations.
...
The local factor is a product of local factors, one for each place of $K$.  In our expresson the local factors at infinite places will be defined in a way which depends on the model, so the complete local factor also includes a correction factor making the expression independent of the model.
...
gives a basis for the period lattice with respect to each infinite place, starting with the real places.  In evaluating `ConjecturalSha()` the product of the actual real and complex periods $\Omega_v$ are computed from these, including a power of 2 for the real places where the discriminant is positive and the factor $2^{r_2}$, but the individual $\Omega_v$ values are not accessible to the user.
:::

:::info
#### Sage

`[EK.period_lattice(pl).omega() for pl in K.places()]`
...
`prod([EK.period_lattice(pl).omega() for pl in K.places()]) * 2**(K.signature()[1])`
:::

## The full formula

|&#1064;| $=$
$$ |d_K|^{1/2} \frac{L^{(r)}(E,1)}{r!} \frac{|T|^2}{R(P_1,\dots,P_r)} N(\mathfrak{u}) \prod_{v\nmid\infty}c_v\prod_{v\mid\infty}\Omega_v.$$


## Comparisons

### Tate
Tate has a factor $|\mu|^g$ where $g$ (which is $d$ in Tate's notation) is the dimention of the variety and he states that $|mu|=|d_K|^{1/2}/2^{r_2}$.   So this accounts for both  our "field factor" $|d_K|^{1/2}$ and also the doubling of the complex periods.

### Gross
Gross states the formula in the form L-value$=M(A)R(A)h(A)$ where $h(A)=\#$&#1064;.  In the analogy with the analytic class number formula &#1064; plays the role of the class group whose order is usually denoted $h$.  The other factors match our Mordell-Weil factor and Local Factors respectively:  $R(E)=R(P_1,\dots,R_r)/|T|^2$ for elliptic curves, adjusted as before for general dimension.

$$M(A) = M_{\infty}(A)M_f(A),$$ where (in our notation) $$M_{\infty}(A) = (\prod_{v\mid\infty}\Omega_v)\cdot N(\mathfrak{u}) / |d_K|^{1/2},$$ and $M_f(A)$ is the Tamagawa product $\prod_{v\nmid\infty}c_v$.
